...
function createChatRoute({ roomManager, agents, agentMap, llmCall, memoryStorage, toolRegistry, audit }) {
  ...
  async function handleAgents(message, history, roomId, activeAgents, res, sendResponse) {
    ...
    const promises = agentList.map(async (agentId) => {
      ...
      try {
        ...
        sendResponse(res, agentId, displayContent, isLast, agentName, agentEmoji);
        responses.push({ agent: agentId, name: agentName, content: displayContent });
  ...
  router.post('/', function(req, res) {
    const body = req.body;
    handleAgents(body.message, body.history || [], body.roomId, body.activeAgents || [], res, function(r, id, content, isLast, agentName, agentEmoji) {
      r.write('data: ' + JSON.stringify({ type: 'response', agent: id, agentName, agentEmoji, content: content }) + '\n\n');
    });
  });
  ...
  router.post('/stream', function(req, res) {
    const body = req.body;
    handleAgents(body.message, body.history || [], body.roomId, body.activeAgents || [], res, function(r, id, content, isLast, agentName, agentEmoji) {
      r.write('data: ' + JSON.stringify({ type: 'token', agent: id, agentName, agentEmoji, token: content }) + '\n\n');
