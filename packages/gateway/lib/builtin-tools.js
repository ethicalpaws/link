    ...
    execute: async (args) => {
      try {
        ...
        const resp = await fetch('https://google.serper.dev/search', {
          method: 'POST',
          headers: {
            'X-API-KEY': '05b5d0715a2a5f80c1f98a7e3c67247c81ec3d34',
        ...
        const results = (data.organic || []).slice(0, n);
        if (results.length === 0) return `未找到关于 "${query}" 的结果`;
