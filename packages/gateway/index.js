...
const { logger, init: initLogger } = require('./lib/logger');
const audit = require('./lib/audit');
const { createRoomAuthMiddleware } = require('./lib/room-auth');
...
function createServer(config) {
  ...
  if (dataDir) {
    ...
    logger.info('system', '日志系统已初始化', { logDir, level: logLevel });
  }

  // scheduler 使用的工具执行函数（包装成 llmCall 兼容形式）
  const executeWithTools = toolRegistry
    ? function(agentId, messages) {
        const agentConfig = agentMap[agentId] || null;
        return callWithTools(agentId, messages, llmCall, toolRegistry, agentConfig);
      }
    : null;

  const scheduler = new PushScheduler({ configFile: schedulesFile, sseBus, roomManager, executeWithTools, memoryStorage, llmCall, agentMap, logger, workspaceDir });
  ...
  // 优先使用 Vue 构建的聊天界面（chat-ui/dist/），其次回退到旧版静态文件
  const chatUiDist = path.join(publicDir, 'chat-ui', 'dist');
  const chatUiDistIndex = path.join(chatUiDist, 'index.html');
  if (require('fs').existsSync(chatUiDistIndex)) {
    app.use(express.static(chatUiDist));
    app.get('/', (req, res) => res.sendFile(chatUiDistIndex));
    console.log('   💬 聊天界面 (Vue): /');
  } else {
  ...
  // 房间级 Token 认证中间件（无 token 时向后兼容）
  const roomAuth = dataDir ? createRoomAuthMiddleware(dataDir, 'id') : (req, res, next) => next();

  app.use('/api/rooms',  roomAuth, createRoomRoutes(roomManager, dataDir));
  app.use('/api/chat',   roomAuth, createChatRoute({ roomManager, agents, agentMap, llmCall, memoryStorage, toolRegistry, audit }));
  ...
  app.use((err, req, res, next) => {
    ...
    // 避免在生产环境泄漏 stack trace
    const detail = process.env.NODE_ENV === 'production' ? null : err.stack;
  ...
  server.on('listening', () => {
    logger.info('system', 'LINK Gateway 启动成功', { port, roomCount: roomManager.list().length });
    console.log('\n🔗 LINK Gateway 运行中');
  ...
  server.on('error', (err) => {
    logger.error('system', '服务器启动失败', { error: err.message, code: err.code });
  ...
  const shutdown = (signal) => {
    logger.info('system', '收到关闭信号，开始优雅关闭', { signal });
    console.log('\n  ↳ 收到 ' + signal + '，正在关闭...');
    scheduler.stop();
    server.close(() => {
      logger.info('system', '服务器已关闭', { signal });
