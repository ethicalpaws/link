...
const allTools = ref([])
const allSkills = ref([])
const form = reactive({ id: '', name: '', emoji: '🤖', autoReply: false, prompt: '', soul: '', agents: '', tools: [], skills: [], toolsmd: '' })
...
function startNew() {
  ...
  Object.assign(form, { id: '', name: '', emoji: '🤖', autoReply: false, prompt: '', soul: '', agents: '', tools: [], skills: [], toolsmd: '' })
}
...
async function save() {
  ...
  try {
    ...
    const cfg = { id: form.id, name: form.name || form.id, emoji: form.emoji || '🤖', autoReply: form.autoReply, prompt: form.prompt, agentDir: './agents/' + form.id }
    if (form.tools.length > 0) cfg.tools = form.tools
    if (form.skills.length > 0) cfg.skills = form.skills
    const writeFiles = { id: form.id }
    if (form.soul) writeFiles.soul = form.soul
    if (form.agents) writeFiles.agents = form.agents
    if (form.toolsmd) writeFiles.toolsmd = form.toolsmd
